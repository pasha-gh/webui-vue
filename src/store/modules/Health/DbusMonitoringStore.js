...
const DbusMonitoringStore = {
  ...
  mutations: {
    ...
    setCaptureStatus: (state, isCapturing) => (
  ...
  actions: {
    async getDbusCaptureDump({ commit }) {
      await api
        .get('/redfish/v1/System/dbus/dbus_capture.json')
        .then(res => {
    ...
    async startDbusCapture({ dispatch, commit }) {
      await api
        .get("/redfish/v1/Systems/dbus/DBusCapture/Actions/DbusCapture.StartCapture/")
        .then(() => dispatch("getDbusCaptureStatus"))
        .then(() => commit("setCaptureStatus", true)) // TODO remove
        .then(() => commit("setCaptureStartTime", Date.now())) // TODO remove
    ...
    async stopDbusCapture({ dispatch, commit }) {
      await api
        .get("/redfish/v1/Systems/dbus/DBusCapture/Actions/DbusCapture.StopCapture/")
        .then(() => dispatch("getDbusCaptureStatus"))
        .then(() => commit("setCaptureStatus", false)) // TODO remove
        .then(() => commit("setCaptureStopTime", Date.now())) // TODO remove
