              ...
              <b-col xl="12">
                <b-form-group label="Group by:">
                  <b-form-checkbox-group id="checkboxes" v-model="form.checked">
                    <b-form-checkbox value="cookie">{{this.$t("pageDbusMonitoring.form.cookie")}}</b-form-checkbox>
                    <b-form-checkbox value="path">{{this.$t("pageDbusMonitoring.form.path")}}</b-form-checkbox>
                    <b-form-checkbox value="sender">{{this.$t("pageDbusMonitoring.form.sender")}}</b-form-checkbox>
                    <b-form-checkbox value="destination">{{this.$t("pageDbusMonitoring.form.destination")}}</b-form-checkbox>
                    <b-form-checkbox value="interface">{{this.$t("pageDbusMonitoring.form.interface")}}</b-form-checkbox>
...
import DbusMonitoringStore from '../../../store/modules/Health/DbusMonitoringStore';
...
export default {
  ...
  methods: {
    ...
    chartDataByType(type) {
      ...
      if (!isNaN(beginTimeStamp) && !isNaN(parseFloat(beginTimeStamp))) {
        filteredEvents = filteredEvents.filter(event => parseFloat(event.time) > parseFloat(beginTimeStamp));
      }
      if (!isNaN(endTimeStamp) && !isNaN(parseFloat(endTimeStamp))) {
        filteredEvents = filteredEvents.filter(event => parseFloat(event.time) < parseFloat(endTimeStamp));
      }
      ...
      const data = filteredEvents.map(event => {
        ...
        if (lodash.has(event, 'reply_event')) {
          end = parseFloat(event.reply_event.time);
        }
        return {
          x: event.cookie + event.path + event.sender + event.destination + event.interface || "",
          y: [
            start,
            end
          ]
  ...
  computed: {
    ...
    chartSeries() {
      return [
        this.chartDataByType('4'),
        this.chartDataByType('1')
      ];
    },
    tableItems() {
      return [{
